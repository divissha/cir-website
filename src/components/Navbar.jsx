...
const navigation = ["Home", "Members", "Workshop", "Internships", "Collaborations", "Students", "Projects", "Gallery", "Donations"];
          ...
          {navigation.map((item) => (
            <a
              ...
              className={`relative text-[14px] font-medium transition ${
                item === "Home" ? "text-blue-900" : "text-[#17213b] hover:text-blue-600"
              }`}
            >
              {item}
              {item === "Home" && (
                <span className="absolute -bottom-[31px] left-0 h-[2px] w-full bg-blue-600" />
              )}
            </a>
          ))}
          <a href="#contact" className="flex items-center gap-2 rounded-lg bg-blue-900 px-5 py-3 text-sm font-semibold text-white shadow-lg shadow-blue-500/20 transition hover:bg-blue-900">
      ...
      {mobileOpen && (
        <div className="border-t border-slate-100 bg-white px-6 py-5 lg:hidden">
          <div className="flex flex-col gap-5">
            {navigation.map((item) => (
              <a key={item} href={`#${item.toLowerCase()}`} onClick={() => setMobileOpen(false)} className="text-sm font-medium text-slate-700">
                {item}
              </a>
            ))}
